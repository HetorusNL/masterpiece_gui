...
class App extends Component {
  ...
  showAllWords = async () => {
    ...
    const res = await axios.get(`https://api.masterpiece.hetorus.nl/words`);
    this.setState({ words: res.data, loading: false });
  };

  // search for words via the masterpiece API
  searchWords = async (text, searchWordOnly) => {
    this.setState({ loading: true });
    this.setState({ showAllWordsButton: true });
    const res = await axios.get(
      `https://api.masterpiece.hetorus.nl/search/word/${
        searchWordOnly ? "dutch,english" : "*"
      }/${text}`
    );
    this.setState({ words: res.data, loading: false });
  };
  ...
  showAllCourseWords = async (course) => {
    ...
    this.setState({ showAllWordsButton: false });
    const res = await axios.get(
      `https://api.masterpiece.hetorus.nl/word/course/${course}`
    );
    this.setState({ words: res.data, loading: false });
  };
  ...
  searchCourseWords = async (course, text, searchWordOnly) => {
    ...
    this.setState({ showAllWordsButton: true });
    const res = await axios.get(
      `https://api.masterpiece.hetorus.nl/course/${course}/search/word/${
        searchWordOnly ? "dutch,english" : "*"
      }/${text}`
    );
    this.setState({ words: res.data, loading: false });
  ...
  // get a single word via the masterpiece API
  getWord = async (id) => {
    this.setState({ loading: true });
    const res = await axios.get(
      `https://api.masterpiece.hetorus.nl/word/id/${id}`
    );
